...
class BitmapUtils {
    ...
    fun ratio(imgPath: String?, pixelW: Float, pixelH: Float): Bitmap {
        ...
        var be = 1 //be=1表示不缩放
        if (w > h && w > pixelW // 设置宽度为120f，可以明显看到图片缩小了){ //如果宽度大的话根据宽度固定大小缩放
                be =(newOpts.outWidth / pixelW // 设置宽度为120f，可以明显看到图片缩小了).toInt()
 ...
 */
@Throws(IOException::class)
fun compressAndGenImage(imgPath: String?, outPath: String?, maxSize: Int, needsDelete: Boolean) {
 ...
 */
@Throws(FileNotFoundException::class)
fun ratioAndGenThumb(imgPath: String?, outPath: String?, pixelW: Float, pixelH: Float, needsDelete: Boolean) {
