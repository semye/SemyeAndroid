...
public class BitmapUtils {
    ...
    public static Bitmap compressQuality(String imagePath, File file) {
    ...
    public static void transImage(String fromFile, String toFile) {
        try {
            Bitmap bitmap = BitmapFactory.decodeFile(fromFile);
            int bitmapWidth = bitmap.getWidth();
            int bitmapHeight = bitmap.getHeight();
            float ratio = (float) Math.sqrt((double) 1100000 / (bitmapWidth * bitmapHeight));
            // 缩放图片的尺寸
            float scaleWidth = ratio * bitmapWidth;
            float scaleHeight = ratio * bitmapHeight;
            float w = scaleWidth / (float) bitmapWidth;
            float h = scaleHeight / (float) bitmapHeight;
     ...
     * @return
     */
    public static String switchSuffix(String fileName) {
