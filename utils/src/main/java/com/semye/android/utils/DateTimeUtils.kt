...
object DateTimeUtils {
    ...
    fun timestampGetYear(timestamp: Long): String {
        val calendar = Calendar.getInstance()
        calendar.time = Date(timestamp)
        return calendar.get(Calendar.YEAR).toString()
    }

    fun timestampFormat1(timestamp: Long): String {
        val dateInstance = SimpleDateFormat.getDateInstance()
        return dateInstance.format(timestamp)
    }

    fun timestampParse(timestamp: String): Date? {
        val dateInstance = SimpleDateFormat.getDateInstance()
        return dateInstance.parse(timestamp)
    }

    fun timestampFormat2(timestamp: Long): String {
        val dateInstance = SimpleDateFormat.getTimeInstance()
        return dateInstance.format(timestamp)
    }
    ...
    fun timestampFormat(timestamp: Long): String {
        val simpleDateFormat1 =
            SimpleDateFormat("yyyy年MM月dd日 HH时mm分ss秒", Locale.getDefault(Locale.Category.FORMAT))
        return simpleDateFormat1.format(timestamp)
    }
    ...
    fun ChangeTime(
        calendar: Calendar, datePicker: DatePicker,
        timePicker: TimePicker, addtime: Int
    ): String {
        var calendar = calendar
        var `val`: String
